...
- **Balanced result (M6) — the headline demo story: the LSTM WINS on structured demand, LOSES on volatility, and both are reported.**
  - **Wins** on the enriched synthetic series (mean MASE ≈ 0.68 vs seasonal-naive ≈ 1.10, **−38%**, at every horizon). The series carries weekly **and** monthly seasonality, AR(1) noise, and shocks — real, learnable structure a *single-lag* seasonal-naive(7) cannot capture but a 28-step LSTM can. **Not rigged:** the win comes from structure the baseline genuinely ignores, verified through the same leak-free walk-forward harness (train-only scaling, last-window-only inference). This is the answer to whiteboard Q1 ("prove it isn't leakage") *and* Q6.
  - **Loses** to the persistence baseline on AAPL realized volatility (mean MASE ≈ 2.7 vs naive ≈ 1.5), because volatility is smooth and highly persistent — "tomorrow ≈ today" is hard to beat and MASE amplifies small absolute errors against a tiny day-to-day-change denominator. The whiteboard Q3/Q4 story, told straight.
  - Net: demonstrates the deep model adding value *where the structure warrants it* and the honesty to say where it doesn't. The synthetic generator was enriched (multi-seasonality + AR + shocks, per SPEC §5; the new args default to off so M1 behavior is unchanged) specifically to give the LSTM a fair series — chosen over leaving both series favoring simple baselines.
